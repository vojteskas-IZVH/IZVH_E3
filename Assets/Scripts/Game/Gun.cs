        ...
        if (shotgun)
        {
            for(var i = 0; i < shotgunBullets; i++)
            {
                SpawnBullet(
                    director.position,
                    director.rotation * 
                    Quaternion.Euler(0.0f, -shotgunSpread/2, 0.0f) * // start of spread
                    Quaternion.Euler(0.0f, i*shotgunSpread/shotgunBullets, 0.0f)// incrementally add bullets to spread
                );
            }
        }
        else
